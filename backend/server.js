...
app.use(cors({
  origin: ['http://localhost:5173', 'http://localhost:3000', 'http://localhost:4173','https://eval-50qea5ca9-mayanks-projects-fd92aa30.vercel.app','https://eval-ai-beta.vercel.app'],
  credentials: true
}));
...
app.post('/evaluate', checkAuth, upload.single('file'), async (req, res) => {
    try {
        ...
        // *** FIX STARTS HERE ***
        // The API expects the 'contents' field to be an array of 'Content' objects.
        // Each 'Content' object must have a 'role' and a 'parts' array.
        // The original code was incorrectly passing an array of 'Part' objects as the 'contents'.
